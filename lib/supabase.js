...
/**
 * Download a file from storage by path. For use by the image proxy (no CORS).
 * Returns { data: ArrayBuffer, contentType: string } or null if not found.
 */
export async function downloadPhotoByPath(path) {
  if (!path) return null;
  const supabase = getClient();
...
/** Upload a cover image for a specific event. Path: covers/events/{eventId}/cover.{ext} */
export async function uploadEventCoverImage(buffer, eventId, ext, contentType) {
  const path = `covers/events/${eventId}/cover.${ext}`;
  const supabase = getClient();
...
export async function getCoverImageUrl(imagePath) {
  ...
  try {
    ...
    if (error) {
      console.error("[getCoverImageUrl] Supabase error:", error.message, "| path:", imagePath);
      return null;
    }
    return data?.signedUrl || null;
  } catch (err) {
    console.error("[getCoverImageUrl] threw:", err.message);
    return null;
  }
}
