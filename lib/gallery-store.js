...
/** Create an album scoped to a specific event. */
export async function createAlbumForEvent(name, eventId, hostId) {
  const slug = makeSlug(name);
  const { data, error } = await db()
    .from("gallery_albums")
    .insert({ name: name.trim(), slug, host_id: hostId, event_id: eventId })
    .select()
    .single();
...
/** List albums shared with an email, scoped to a specific event. */
export async function listAlbumsForEmailByEvent(email, eventId) {
  const { data: shares } = await db()
    .from("gallery_album_shares")
    .select("album_id")
    .ilike("email", email.trim());
...
/** List all albums for a specific event. */
export async function listAlbumsByEvent(eventId) {
  const { data, error } = await db()
    .from("gallery_albums")
    .select("*, gallery_album_shares(email)")
    .eq("event_id", eventId)
...
export async function updateAlbumSlideshowEnabled(albumId, enabled, hostId) {
  const { error } = await db()
    .from("gallery_albums")
