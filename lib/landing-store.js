...
/** Convert www.pexels.com/photo/... page URLs to images.pexels.com CDN (avoids 403 when Next fetches). */
function toPexelsCdnUrl(url) {
  if (!url || typeof url !== "string") return url;
  const m = url.match(/^https:\/\/www\.pexels\.com\/photo\/[^/]+-(\d+)\/?/);
...
function mergeWithDefault(row) {
  ...
  const hero = { ...base.hero, ...(dbConfig.hero || {}) };
  // Normalize hero photo URLs: www.pexels.com returns 403 when Next fetches; use CDN
  if (Array.isArray(hero.photos)) {
    hero.photos = hero.photos.map((p) => (p && p.src ? { ...p, src: toPexelsCdnUrl(p.src) } : p));
  }
  return {
    ...base,
    id: row.id,
    ...
    eventTypeKeys: Array.isArray(dbConfig.eventTypeKeys) ? dbConfig.eventTypeKeys : (base.eventTypeKeys || []),
    intakeMode:    dbConfig.intakeMode   ?? base.intakeMode ?? "light",
    helpCta:       { ...(base.helpCta || {}), ...(dbConfig.helpCta || {}) },
  };
}
